...
cellCheck = function(row) {
  var cell_data = $('#program').handsontable('getDataAtRow',row);
...
$("#program").handsontable({
  startRows: 256,
  startCols: 1,
  rowHeaders: ["0x00","0x01","0x02","0x03","0x04","0x05","0x06","0x07","0x08","0x09","0x0A","0x0B","0x0C","0x0D","0x0E","0x0F",
...
$writeButton.click(function(event) {
  ...
  for(var i=0;i<256;i++){
    if(!cellCheck(i)){
      return console.log("%d false",i);
    }
  }
  serial.startWriteReplyReceive();
...
writeReplyReceiveCallback = function(data) {
  var table = $('#program').handsontable('getDataAtCol',0);
  var program = new Array(256);
  for(var i=0; i<256; i++){
    program[i] = parseInt(table[i],16);
  }
  console.log("Receive data");
  if(data == "ff"){
    return serial.sendData(program);
  }else{
...
readReceiveCallback = function(data) {
  buff=buff+data;
  console.log("Receive data");
  var program = buff.split(",");
